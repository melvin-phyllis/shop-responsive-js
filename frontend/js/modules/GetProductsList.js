        ...
        listesProducts.forEach(item => {
           ...
           <div class="card col-span-12 md:col-span-6 lg:col-span-3 bg-white border border-gray-200 rounded-lg hover:shadow-xl transition-all duration-300 group">
            ...
            <div class="card-body p-5 space-y-3">
                <h2 class="text-base font-bold text-gray-900 uppercase tracking-wider">${item.description}</h2>
                ...
                <div class="card-actions grid grid-cols-2 gap-2 pt-2">
                    <button data-id="${item.id}" class="bg-red-600 text-white text-xs font-semibold py-2 rounded hover:bg-red-700 transition-colors">Delete</button>
                    <button data-id="${item.id}" class="bg-white border border-gray-300 text-gray-900 text-xs font-semibold py-2 rounded hover:bg-gray-100 transition-colors" onclick="my_modal_2.showModal()">View</button>
                    <button data-id="${item.id}" class="bg-gray-900 text-white text-xs font-semibold py-2 rounded hover:bg-black transition-colors" onclick="my_modal_3.showModal()">Edit</button>
                    <button data-id="${item.id}" class="bg-black text-white text-xs font-semibold py-2 rounded hover:bg-gray-800 transition-colors">Sell</button>
                </div>
            </div>
        </div>`
        });
